<div class="flex ">
    <form action="/tweets/{{$tweet->id}}/like" method="POST">
        @csrf
        <div class="flex items-center  mt-4 mr-4 {{ $tweet->isLikedBy(current_user()) ? 'text-blue-500' : 'text-gray-500'}}">
    ...
    <form action="/tweets/{{$tweet->id}}/like" method="POST">
        @csrf
        @method('DELETE')
        <div class="flex items-center  mt-4 {{$tweet->isDislikedBy(current_user()) ? 'text-blue-500' : 'text-gray-500'}} ">
            <svg viewBox="0 0 20 20" class=" mr-1 w-3">
                <g id="Page-1" stroke="none" stroke-width="1" fill="none" fill-rule="evenodd">
                    <g class="fill-current">
            ...
            <button class="text-xs text-gray-500">{{$tweet->dislikes ?: 0}}</button>
        </div>
    </form>
